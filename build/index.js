...
const isValidInteractionArgs = (args) => {
    return (typeof args === 'object' &&
        args !== null &&
...
class ReactomeServer {
        ...
        this.server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
            ...
            if (pathwayMatch) {
                ...
                try {
                    ...
                    throw new McpError(ErrorCode.InternalError, `Failed to fetch pathway ${pathwayId}: ${error instanceof Error ? error.message : 'Unknown error'}`);
                }
            }
            ...
            if (reactionMatch) {
                ...
                try {
                    ...
                    throw new McpError(ErrorCode.InternalError, `Failed to fetch reaction ${reactionId}: ${error instanceof Error ? error.message : 'Unknown error'}`);
                }
            }
    ...
    async handleSearchPathways(args) {
        ...
        try {
            ...
            if (args.type) {
                ...
                allEntries = allEntries.filter((entry) => entry.exactType?.toLowerCase().includes(typeFilter) ||
    ...
    async resolvePathwayId(identifier) {
    ...
    async handleGetPathwayDetails(args) {
    ...
    async handleFindPathwaysByGene(args) {
        ...
        try {
            ...
            if (args.species) {
                proteinEntries = proteinEntries.filter((entry) => Array.isArray(entry.species) ?
                    entry.species.some((s) => s.toLowerCase().includes(args.species.toLowerCase())) :
            ...
            const result = {
                ...
                pathways: pathwaysResponse.data?.map((pathway) => ({
                    id: pathway.stId,
                    name: pathway.name,
    ...
    async handleFindPathwaysByDisease(args) {
        ...
        try {
            ...
            const diseasePathways = {
                ...
                pathways: pathwayEntries.map((pathway) => ({
    ...
    async handleGetPathwayHierarchy(args) {
        if (!isValidIdArgs(args)) {
            throw new McpError(ErrorCode.InvalidParams, 'Pathway ID is required');
        }
        try {
            ...
            try {
                ...
                if (pathwayInfo.data.hasEvent) {
                    children = pathwayInfo.data.hasEvent.map((event) => ({
            ...
            const hierarchy = {
                ...
                ancestors: ancestors.length > 0 ? ancestors.slice(0, 10).map((ancestor) => ({
    ...
    async handleGetPathwayParticipants(args) {
        if (!isValidIdArgs(args)) {
            throw new McpError(ErrorCode.InvalidParams, 'Pathway ID is required');
        }
        try {
            ...
            try {
                ...
                try {
                    ...
                    if (pathwayDetails.data.hasEvent) {
                        participants = pathwayDetails.data.hasEvent.map((event) => ({
            ...
            const result = {
                ...
                participants: participants.slice(0, 50).map((participant) => ({
                    id: participant.stId,
    ...
    async handleGetPathwayReactions(args) {
        if (!isValidIdArgs(args)) {
            throw new McpError(ErrorCode.InvalidParams, 'Pathway ID is required');
        }
        try {
            ...
            const reactions = response.data?.filter((event) => event.schemaClass === 'Reaction' || event.schemaClass === 'BlackBoxEvent') || [];
    ...
    async handleGetProteinInteractions(args) {
        ...
        try {
            ...
            try {
                ...
                proteins = participantsResponse.data?.filter((p) => p.schemaClass === 'EntityWithAccessionedSequence' || p.schemaClass === 'Protein') || [];
            }
            catch (e1) {
                ...
                try {
                    ...
                    if (pathwayInfo.data.hasEvent) {
                        proteins = pathwayInfo.data.hasEvent
                            .filter((event) => event.schemaClass?.includes('Protein') || event.schemaClass?.includes('Entity'))
                            .slice(0, 5);
                    }
                }
            }
            try {
                ...
                reactions = reactionsResponse.data?.filter((event) => event.schemaClass === 'Reaction') || [];
            }
            catch (e) {
                // Extract reactions from pathway events
                if (pathwayInfo.data.hasEvent) {
                    reactions = pathwayInfo.data.hasEvent
                        .filter((event) => event.schemaClass === 'Reaction')
                        .slice(0, 10);
                }
            }
            const interactions = {
